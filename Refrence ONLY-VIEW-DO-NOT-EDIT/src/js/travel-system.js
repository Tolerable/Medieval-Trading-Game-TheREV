...
TravelSystem.showTravelPanel = function() {
    ...
    if (travelPanel) {
        ...
        // 🖤 ALWAYS default to destinations tab - let the HTML default stand
        // Only explicitly switch to map if user has no destination AND wants to pick one
        // For now, just ensure destinations tab is active by default

        // Update tab buttons - destinations is default
        document.querySelectorAll('.travel-tab-btn').forEach(btn => {
            btn.classList.remove('active');
        });
        const destinationsBtn = document.querySelector('[data-travel-tab="destinations"]');
        if (destinationsBtn) {
            destinationsBtn.classList.add('active');
        }

        // Update tab content visibility - destinations is default
        ...
        if (destinationsTab) {
            destinationsTab.classList.add('active');
        }

        // Render the mini map if it exists (for when user switches to map tab)
        if (typeof TravelPanelMap !== 'undefined' && TravelPanelMap.render) {
            setTimeout(() => TravelPanelMap.render(), 50);
        }
    }
};
